...
"""
Usage Guide for train_network.py
...
Basic Usage:
    python3 train_network.py <path/to/config.yaml>
...
Examples:
    ...
    python3 train_network.py <path/to/config.yaml> --verbose 0

    Train and save a new model with high verbosity:
    python3 train_network.py <path/to/config.yaml> --verbose 2 --save <path/to/save/model>
    python3 train_network.py config.yml
"""
...
def build_model(config):
    ...
    regularization_type = regularization_functions[config['network']['regularization']['type']]
...
def generate_data(config):
    dataset_split = (config['data']['split']['train'], config['data']['split']['val'], config['data']['split']['test'])
    generator = Generator(image_size=config['data']['size'],
                          total_images=config['data']['quantity'],
                          dataset_split=dataset_split,
                          flatten=config['data']['flatten'])
    train_img, val_img, test_img = generator.generate(noise_level=config['data']['noise'])
